...
**Status:** V13.2 (April 30 2026) — admin /admin/costs accuracy + auto-refresh + DB performance hardening. Per-call cost attribution moved into `apps/web/lib/usage/cost-attribution.ts` (one helper per provider; prefers actual usage; falls back to configured formulas/constants; explicitly forbidden from balance-delta attribution). New columns on `ApiCall` (`estimatedCostUsd` / `actualCostUsd` / `metadata` / `renderJobId` / `sceneId`), new `ProviderBalanceSnapshot` model + 60s in-process cache, five new admin API endpoints under `/api/admin/costs/*` (server-side admin guarded via `requireAdminApi()` returning JSON 401/403), three polling client components on the dashboard (summary 20s · in-flight 4s · recent-50 8s) with date + provider + operation + status filters, 13 new composite indexes. 390+ V13 assertions pass. V13.1 patch: non-lipsync scene clips now actually carry voice audio in production — the prior bundler issues that left ffmpeg-static missing from the Vercel function are routed around via a cold-start CDN download to `/tmp`. Mux failures no longer ship silent clips OR charge the user. PR1 removed the post-generation Image QA auto-regen loop; PR2 strengthened the upstream Image Brief (Israeli realism / hands physics / mirror safety / product reference lock / product-demo contact proof); PR3 added the deterministic Animation Plan that drives Kling's `{ positive, negative }`; PR4 added a stage-tagged logger with sensitive-data masking; PR5 shipped a curated Hebrew error map; PR6 added the state-machine schema (additive migration `v13_scene_state_log`); PR7 wired state-machine writes + per-scene log persistence + 4 wizard UX components; PR8 surfaced every persisted artifact at `/admin/scenes/[id]/debug`; PR9 ships `npm test`.
